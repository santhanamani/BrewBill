...
from ..models import (
    Category, GlobalProduct, Inventory, Outlet, OutletProductMapping, Product, ProductVariant, User,
)
from ..schemas import (
...
def mapping_view(session: Session, mapping: OutletProductMapping) -> OutletProductMappingRead:
    inventory = inventory_for(
        session, mapping.tenant_id, mapping.outlet_id, mapping.legacy_product
    )
    master = mapping.global_product
    return OutletProductMappingRead(
        id=mapping.id,
        global_product_id=master.id,
        legacy_product_id=mapping.legacy_product_id,
        code=master.code,
        name=mapping.outlet_specific_name or master.name,
        category_name=master.category_name,
        image_path=master.image_path,
        base_unit=master.base_unit,
        default_gst=mapping.tax_override if mapping.tax_override is not None else master.default_gst,
...
def list_outlet_catalogue(
    ...
    rows = [mapping_view(session, row) for row in mappings]
    session.commit()
    return rows
...
def list_products(
    ...
    rows = [product_view(
        product, inventory_for(session, user.tenant_id, selected_outlet, product),
